...
public interface SkillLevelRepository extends CrudRepository<SkillLevel, Long> 
{
	...
	@Query(value= "select lv from SKILL_LEVELS as lv where lv.value >= :value order by lv.value asc", nativeQuery=true)
	public List<SkillLevel> findWithGreaterOrEqual(@Param("value")Integer value);
	
	@Query(value="select lv from SKILL_LEVELS as lv where lv.value > :value order by lv.value asc limit 1", nativeQuery=true)
	...
	@Query(value="select lv from SKILL_LEVELS as lv where lv.value > :lower and lv.value <= :upper order by lv.value asc", nativeQuery=true)
