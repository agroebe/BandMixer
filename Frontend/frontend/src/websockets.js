...
const host = process.env.NODE_ENV === 'production' ? window.location.host : 'coms-309-cy-01.cs.iastate.edu:8080/chat/'
...
// to the server
export const startWebsocketConnection = (user) => {
  // A new Websocket connection is initialized with the server
  const ws = new window.WebSocket('ws://' + host + user) || {}
